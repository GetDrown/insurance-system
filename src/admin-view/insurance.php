...
<body>
    ...
    <dialog data-policy-modal>
        <div>
            ...
            <form action="../../phpscript/addinsurances.php" method="POST" class="flex flex-col px-3">
                <!-- username -->
                ...
                <div class="h-[35px] flex mb-5">
                    <div class="basis-1/5  bg-gray-700 flex justify-center items-center h-full text-white rounded-tl-md rounded-bl-md text-sm px-2">
                        Quantity</div>
                    <input name="policy_quantity" type="text" class="basis-4/5  h-full border-2 px-3 py-2 rounded-tr-md rounded-br-md active:outline-none focus:outline-none">
                </div>
                <!-- buttns -->
                <div class="flex justify-around items-center ">
                    <button type="submit" class="bg-lime-300 py-2 px-2 rounded-lg font-medium w-[100px] shadow-md">Save</button>
                    <button data-close-policy-modal class="bg-red-300 py-2 px-2 rounded-lg font-medium w-[100px] shadow-md">Cancel</button>
                </div>
            </form>
        </div>
    </dialog>
    ...
    <div class="basis-5/6  h-dvh p-3 rounded-lg overflow-hidden bg-gray-200 flex flex-col">
        ...
        <div class="h-full rounded-md overflow-y-auto bg-white drop-shadow-md p-5 grid grid-cols-1 gap-5 relative" id="clientContainer">
            ...
            <?php foreach ($allPolicy as $i => $policy) { ?>
                <div class="client bg-white h-[80px] rounded-md drop-shadow-md p-5 flex justify-between items-center border-t border-gray-200">

                    <div>
                        <p class="text-sm font-medium text-gray-400 normal-case">No.</p>
                        <p class="text-[22px] font-medium text-gray-600 normal-case"><?php echo $i + 1; ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-400 normal-case">Name.</p>
                        <p class="text-[20px] font-normal text-gray-600 normal-case"><?php echo isset($policy['non_life_name']) ? $policy['non_life_name'] : $policy['life_desc']; ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-400 normal-case">Type</p>
                        <p class="text-[22px] font-medium text-gray-600 normal-case"><?php echo isset($policy['policy_type']) ? $policy['policy_type'] : $policy['life_policy_type']; ?></p>
                    </div>
                    <div>
                        <p class="text-sm font-medium text-gray-400 normal-case">Quantity</p>
                        <p class="text-[22px] font-medium text-gray-600 normal-case"><?php echo isset($policy['non_life_qty']) ? $policy['non_life_qty'] : $policy['life_qty']; ?></p>
                    </div>
